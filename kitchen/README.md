...
├── kitchen.yml
├── manifests
├── modules `should contain fortishield-puppet module`
├── Puppetfile
├── run.sh
├── test
```
...
```
vagrant@master:~/fortishield-puppet/kitchen$ cat Gemfile
...
```
vagrant@master:~/fortishield-puppet/kitchen$ cat kitchen.yml
---
driver:
  name: docker
...
This way we will only have our machines created without installing the desired components to be tested. These components are represented by Fortishield stack components such as `fortishield-manager`, `fortishield-agent`, etc ...
...
As you can see, we only want to install `fortishield-manager` and `fortishield-agent`.
...
In the folder test/base, we put our tests. By now we implemented 2 tests, one for `fortishield-manager` and another one for `fortishield-agent`. Please check both here: 
* [manager](https://github.com/fortishield/fortishield-puppet/blob/v3.9.5_7.2.1/kitchen/test/base/test_fortishield_manager.py)
* [agent](https://github.com/fortishield/fortishield-puppet/blob/v3.9.5_7.2.1/kitchen/test/base/test_fortishield_agent.py)
...
rootdir: /home/vagrant/fortishield-puppet/kitchen
plugins: testinfra-3.0.5
collecting ... collected 8 items

test/base/test_fortishield_agent.py::test_fortishield_agent_package SKIPPED          [ 12%]
test/base/test_fortishield_agent.py::test_fortishield_processes_running[ossec-agentd-ossec] SKIPPED [ 25%]
test/base/test_fortishield_agent.py::test_fortishield_processes_running[ossec-execd-root] SKIPPED [ 37%]
test/base/test_fortishield_agent.py::test_fortishield_processes_running[ossec-syscheckd-root] SKIPPED [ 50%]
test/base/test_fortishield_agent.py::test_fortishield_processes_running[fortishield-modulesd-root] SKIPPED [ 62%]
test/base/test_fortishield_manager.py::test_fortishield_agent_package PASSED         [ 75%]
test/base/test_fortishield_manager.py::test_fortishield_packages_are_installed PASSED [ 87%]
test/base/test_fortishield_manager.py::test_fortishield_services_are_running PASSED  [100%]
