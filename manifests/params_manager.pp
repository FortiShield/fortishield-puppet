...
# Paramas file
class fortishield::params_manager {
  case $::kernel {
    'Linux': {
    ...
    ### Ossec.conf blocks
      ...
      $ossec_emailto                                   = ['recipient@example.fortishield.github.io']
      $ossec_smtp_server                               = 'smtp.example.fortishield.github.io'
      $ossec_emailfrom                                 = 'ossecm@example.fortishield.github.io'
    ...
    # ossec.conf templates paths
      $ossec_manager_template                          = 'fortishield/fortishield_manager.conf.erb'
      $ossec_rootcheck_template                        = 'fortishield/fragments/_rootcheck.erb'
      $ossec_wodle_openscap_template                   = 'fortishield/fragments/_wodle_openscap.erb'
      $ossec_wodle_cis_cat_template                    = 'fortishield/fragments/_wodle_cis_cat.erb'
      $ossec_wodle_osquery_template                    = 'fortishield/fragments/_wodle_osquery.erb'
      $ossec_wodle_syscollector_template               = 'fortishield/fragments/_wodle_syscollector.erb'
      $ossec_wodle_docker_listener_template            = 'fortishield/fragments/_wodle_docker_listener.erb'
      $ossec_vulnerability_detection_template          = 'fortishield/fragments/_vulnerability_detection.erb'
      $ossec_vulnerability_indexer_template            = 'fortishield/fragments/_vulnerability_indexer.erb'
      $ossec_sca_template                              = 'fortishield/fragments/_sca.erb'
      $ossec_syscheck_template                         = 'fortishield/fragments/_syscheck.erb'
      $ossec_default_commands_template                 = 'fortishield/default_commands.erb'
      $ossec_localfile_template                        = 'fortishield/fragments/_localfile.erb'
      $ossec_ruleset_template                          = 'fortishield/fragments/_ruleset.erb'
      $ossec_auth_template                             = 'fortishield/fragments/_auth.erb'
      $ossec_cluster_template                          = 'fortishield/fragments/_cluster.erb'
      $ossec_active_response_template                  = 'fortishield/fragments/_default_activeresponse.erb'
      $ossec_syslog_output_template                    = 'fortishield/fragments/_syslog_output.erb'
      ...
      $ossec_cluster_name                              = 'fortishield'
      ...
      $local_decoder_template              = 'fortishield/local_decoder.xml.erb'
      $decoder_exclude                     = []
      $local_rules_template                = 'fortishield/local_rules.xml.erb'
      $rule_exclude                        = []
      $shared_agent_template               = 'fortishield/ossec_shared_agent.conf.erb'

      $fortishield_manager_verify_manager_ssl    = false
      $fortishield_manager_server_crt            = undef
      $fortishield_manager_server_key            = undef


      ## Fortishield config folders and modes
      ...
      $config_mode = '0640'
      $config_owner = 'root'
      $config_group = 'fortishield'

      $keys_file = '/var/ossec/etc/client.keys'
      $keys_mode = '0640'
      $keys_owner = 'root'
      $keys_group = 'fortishield'
      ...
      $processlist_group = 'fortishield'

      #API

      $fortishield_api_host = '0.0.0.0'
      $fortishield_api_port = '55000'

      $fortishield_api_file =  undef

      # Advanced configuration
      $fortishield_api_https_enabled = 'yes'
      $fortishield_api_https_key = 'server.key'
      $fortishield_api_https_cert = 'server.crt'
      $fortishield_api_https_use_ca = 'False'
      $fortishield_api_https_ca = 'ca.crt'
      $fortishield_api_ssl_protocol = 'TLSv1.2'
      $fortishield_api_ssl_ciphers  = '""'

      # Logging configuration
      # Values for API log level: disabled, info, warning, error, debug, debug2 (each level includes the previous level).
      $fortishield_api_logs_level = 'info'
      # Values for API log format: 'plain', 'json', 'plain,json', 'json,plain'
      $fortishield_api_logs_format = 'plain'

      # Cross-origin resource sharing: https://github.com/aio-libs/aiohttp-cors#usage
      $fortishield_api_cors_enabled = 'no'
      $fortishield_api_cors_source_route = '"*"'
      $fortishield_api_cors_expose_headers = '"*"'
      $fortishield_api_cors_allow_headers = '"*"'
      $fortishield_api_cors_allow_credentials = 'no'

      # Cache (time in seconds)
      $fortishield_api_cache_enabled = 'yes'
      $fortishield_api_cache_time = '0.750'

      # Access parameters
      $fortishield_api_access_max_login_attempts = 5
      $fortishield_api_access_block_time = 300
      $fortishield_api_access_max_request_per_minute = 300

      # Drop privileges (Run as ossec user)
      $fortishield_api_drop_privileges = 'yes'

      # Enable features under development
      $fortishield_api_experimental_features = 'no'
      ...
      # Fortishield API template path
      $fortishield_api_template = 'fortishield/fortishield_api_yml.erb'


      case $::osfamily {
        'Debian': {

          $agent_service  = 'fortishield-agent'
          $agent_package  = 'fortishield-agent'
          ...
          case $::lsbdistcodename {
            'xenial': {
              $server_service = 'fortishield-manager'
              $server_package = 'fortishield-manager'
    ...
    'windows': {
      ...
      $agent_service  = 'FortishieldSvc'
      $agent_package  = 'Fortishield Agent 5.0.0'
      $server_service = ''
      $server_package = ''
      $api_service = ''
      $api_package = ''
